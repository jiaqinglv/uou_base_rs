...
impl DefaultLogger {
    /// opentelemetry
    pub fn new_tracing_opentelemetry(
        level: log::Level,
        tracer_name: String,
        ...
        let logger_provider = opentelemetry_sdk::logs::LoggerProvider::builder()
            .with_config(opentelemetry_sdk::logs::Config::default().with_resource(
                opentelemetry_sdk::Resource::new(vec![KeyValue::new("service.name", tracer_name)]),
            ))
        ...
        Ok(DefaultLogger { level })
    }

    /// opentelemetry
    pub fn new_tracing_opentelemetry_jaeger(
        level: log::Level,
        tracer_name: String,
    ) -> Result<Self, Box<dyn Error>> {
